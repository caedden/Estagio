...
export default function TableRegister() {
    ...
    const [status, setStatus] = useState(true); // Alterado para booleano (true/false)
    ...
    const handleAddTable = async () => {
        ...
        try {
            const response = await fetch('http://localhost:3000/api/mesa', {
                ...
                body: JSON.stringify({ identifier: tableNumber, status }), // Enviando status como booleano
            });
    ...
    const toggleTableStatus = async (id) => {
        ...
        const newStatus = table.status ? false : true; // Alterando para um valor booleano
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/mesa/${id}`, {
                ...
                body: JSON.stringify({ status: newStatus }), // Enviando status como booleano
            });
                    ...
                    {tables.map((table) => (
                        <li key={table.id} className="table-register-item">
                            ...
                            <div className="table-register-actions">
                                <span
                                    className={`table-status ${table.status ? 'active' : 'inactive'}`}
                                >
                                    {table.status ? 'Ativo' : 'Inativo'} {/* Exibindo o status como texto */}
                                </span>
                                <button
                                    onClick={() => toggleTableStatus(table.id)}
                                    className="table-status-toggle-button"
                                >
                                    {table.status ? `Inativar Mesa ${table.identifier}` : `Ativar Mesa ${table.identifier}`}
                                </button>
                            </div>
                        </li>
                    ))}
